...
import { UserPlus, Search, Pencil, Trash2, Eye, User, Briefcase, CreditCard, MapPin, Camera, Users, Crown, Building2, TrendingUp, Sparkles, Upload, Download, CheckCircle2, XCircle, FileSpreadsheet } from "lucide-react";
import { useRouter } from "next/navigation";
import { Pagination } from "@/components/ui/pagination";
import { EmployeeAvatar } from "@/components/employee-avatar";
import { downloadEmployeeImportTemplate } from "@/lib/export-utils";
...
export default function EmployeesPage() {
  ...
  const [importResult, setImportResult] = useState<{ success: number; failed: number; failedRows: { row: number; name: string; error: string }[] } | null>(null);
  ...
  const handleImport = async () => {
    ...
    if (data.success > 0) fetchEmployees(1, search);
  };
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 bg-white/15 hover:bg-white/25 text-white font-semibold px-4 h-11 rounded-xl border border-white/20 transition-all"
            >
              <FileSpreadsheet className="h-4 w-4" />
              <span className="text-sm">{t("استيراد Excel")}</span>
            </button>
            <button
              onClick={openAdd}
        ...
        <DialogContent className="max-w-lg">
          ...
          {!importResult ? (
            <div className="space-y-4 py-2">
              {/* تحميل النموذج */}
              <div className="rounded-xl border border-dashed border-sky-300 bg-sky-50 dark:bg-sky-950/30 p-4 flex items-start gap-3">
                <Download className="h-5 w-5 text-sky-500 mt-0.5 shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-sky-800 dark:text-sky-300">{t("حمّل النموذج أولاً")}</p>
                  <p className="text-xs text-sky-600 dark:text-sky-400 mt-0.5">{t("أدخل بيانات الموظفين في النموذج ثم ارفعه")}</p>
                  <button
                    onClick={downloadEmployeeImportTemplate}
                    className="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-sky-700 dark:text-sky-300 hover:underline"
                  >
              ...
              {/* رفع الملف */}
              <div>
                <Label className="mb-2 block text-sm">{t("رفع ملف Excel")}</Label>
                <label className="flex flex-col items-center gap-2 border-2 border-dashed border-muted-foreground/30 rounded-xl p-6 cursor-pointer hover:border-brand-primary/50 hover:bg-brand-primary/5 transition-all">
                  <Upload className="h-8 w-8 text-muted-foreground/50" />
                  <span className="text-sm text-muted-foreground">
                    {importFile ? importFile.name : t("اضغط لاختيار ملف .xlsx أو .xls")}
                  </span>
                  <input
                    type="file"
                    accept=".xlsx,.xls"
              ...
              <p className="text-xs text-muted-foreground">{t("الحد الأقصى: 500 موظف في كل ملف")}</p>
            </div>
          ) : (
            <div className="space-y-4 py-2">
              {/* النتائج */}
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 p-4 text-center">
                  ...
                  <p className="text-2xl font-black text-green-700 dark:text-green-400">{importResult.success}</p>
                  <p className="text-xs text-green-600 dark:text-green-400">{t("تمت الإضافة")}</p>
                </div>
                <div className="rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 p-4 text-center">
                  ...
                  <p className="text-2xl font-black text-red-600 dark:text-red-400">{importResult.failed}</p>
                  <p className="text-xs text-red-500 dark:text-red-400">{t("فشل الاستيراد")}</p>
                </div>
              </div>

              {importResult.failedRows.length > 0 && (
                <div className="rounded-xl border border-red-200 dark:border-red-800 overflow-hidden">
                  <div className="bg-red-50 dark:bg-red-950/30 px-3 py-2 text-xs font-semibold text-red-700 dark:text-red-400">
                    {t("تفاصيل الأخطاء")}
                  </div>
                  <div className="divide-y divide-red-100 dark:divide-red-900 max-h-48 overflow-y-auto">
                    {importResult.failedRows.map(r => (
                      <div key={r.row} className="px-3 py-2 text-xs flex gap-2">
                        <span className="text-muted-foreground shrink-0">{t("صف")} {r.row}:</span>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={closeImport}>{t("إغلاق")}</Button>
            {!importResult && (
              <Button onClick={handleImport} disabled={!importFile || importLoading} className="min-w-28">
                {importLoading ? t("جاري الاستيراد...") : t("استيراد")}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
