...
import { Megaphone, Plus, Pencil, Trash2, Globe, Building2 } from "lucide-react";
...
type UserData = { id: string; employee?: { id: string; firstName: string; lastName: string; department?: string; position?: string } };
...
const emptyForm = { title: "", content: "", scope: "company", department: "", priority: "normal" };
...
export default function ManagerAnnouncementsPage() {
  ...
  // فلتر: يرى المدير إعلاناته فقط
  const myAnnouncements = announcements.filter(a => a.authorId === user?.employee?.id);

  const openAdd = () => {
    setEditId(null);
    setForm({ ...emptyForm, department: user?.employee?.department ?? "" });
    setOpen(true);
  };

  const openEdit = (a: Announcement) => {
    setEditId(a.id);
    setForm({ title: a.title, content: a.content, scope: a.scope, department: a.department ?? "", priority: a.priority });
    setOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const authorName = user?.employee ? `${user.employee.firstName} ${user.employee.lastName}` : "";
    const authorId = user?.employee?.id ?? "";
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">إعلاناتك للموظفين</p>
          </div>
        </div>
      ...
      {myAnnouncements.length === 0 ? (
        <Card>
          <CardContent className="py-16 text-center text-gray-400">
            ...
            <Button variant="outline" className="mt-3" onClick={openAdd}>أنشئ إعلانك الأول</Button>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        ...
                        {a.scope === "company" ? (
                          <Badge variant="outline" className="text-xs gap-1"><Globe className="h-3 w-3" /> الشركة كاملة</Badge>
                        ) : (
                          <Badge variant="outline" className="text-xs gap-1"><Building2 className="h-3 w-3" /> {a.department}</Badge>
                        )}
                        {!a.active && <Badge variant="secondary" className="text-xs">متوقف</Badge>}
                      </div>
                      ...
                      <p className="text-xs text-gray-400 mt-2">{new Date(a.createdAt).toLocaleDateString("ar-SA")}</p>
                    </div>
        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={saving || !form.title || !form.content}>
              {saving ? "جارٍ النشر..." : editId ? "حفظ التعديلات" : "نشر الإعلان"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
